	...
	*/
    ...
    for (int i = 0; i < numberOfFaces; i++)
    {
           ...
           //face vertices -in clockwise render order
 	       m_textureVertices[6*i+0].position = bottomVertices[i];  //bottom
	       m_textureVertices[6*i+0].texture = XMFLOAT2(faceWidth * i, 0.0f);	
		   m_textureVertices[6*i+1].position = topCenter;   //top right
	       m_textureVertices[6*i+1].texture = XMFLOAT2(faceWidth * (i + 1), 0.0f);
	       m_textureVertices[6*i+2].position = bottomVertices[i+1];   //bottom left
	       m_textureVertices[6*i+2].texture = XMFLOAT2(faceWidth * i, 1.0f);

		   m_textureVertices[6*i+3].position = bottomCenter;  //bottom left
	       m_textureVertices[6*i+3].texture = XMFLOAT2(faceWidth * i, 1.0f);	
	       m_textureVertices[6*i+4].position = bottomVertices[i];   //top right
		   ...
		   //top slice triangle
		   m_topTextureVertices[3*i+0].position = topCenter;  
	       ...
	       m_topTextureVertices[3*i+2].position = topCenter;   
	...
	m_VertexModelArray[1] = new ModelClass(
				 ...
				 1,//numberOfTopVertices,  //vertex count
				 m_topIndices, 
				 1, //numberOfTopVertices, //index count
				 D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
