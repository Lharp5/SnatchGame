...
DirectInputClass::DirectInputClass(const DirectInputClass& other)
{
}
...
bool DirectInputClass::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	HRESULT result;
...
//The Frame function for the DirectInputClass will read the current state of the devices into state buffers we setup. After the state of each device is read it then processes the changes.

bool DirectInputClass::Frame()
{
	bool result;
...
bool DirectInputClass::ReadKeyboard()
{
	HRESULT result;
...
bool DirectInputClass::ReadMouse()
{
	HRESULT result;
...
//I have added a function to the DirectInputClass called IsEscapePressed. This shows how to utilize the keyboard to check if specific keys are currently being pressed. You can write other functions to check for any other keys that are of interest to your application.

bool DirectInputClass::IsEscapePressed()
{
	// Do a bitwise and on the keyboard state to check if the escape key is currently being pressed.
	if(m_keyboardState[DIK_ESCAPE] & 0x80)
	{
		wchar_t* outstring = L"ESCAPE\n";
...
bool DirectInputClass::keyPressed(unsigned char key)
{
...
void DirectInputClass::GetMouseLocation(int& mouseX, int& mouseY)
{
	mouseX = m_mouseX;
	mouseY = m_mouseY;
	return;
